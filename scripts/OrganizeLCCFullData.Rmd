...
lcc_combustNewBD = read.csv("../data/lcc_combustion/2025-10-28_LCC_CombustionModelPredictors_predictions_LCC_11_18.csv")
...
lcc_fa = read_csv("../outputs/2025-08-18_cleaned/csv/2025-11-19_LCC_ForestAge.csv")
```
...
( lcc_combustNewBD1 = lcc_combustNewBD %>% rename(ID = id) %>% dplyr::select(ID, pred_burn_depth_LCC_11_18) )
( lcc_id1 = lcc_id %>%
...
```{r}
lcc_sa_fieldComputedHybrid <- lcc %>%
  left_join(standAge_field, by = "site") %>%
  #dplyr::select(ID, standAge, fieldAge) %>%
  rename(computedAge = standAge) %>%
  mutate(hybridAge = coalesce(fieldAge, computedAge)) %>% 
    filter(year_of_burn >2000) %>% 
    filter(year_of_burn != "unknown") %>%
    filter(!is.na(FWI)) %>%
    rename(fa00 = ForestAge_TC000, fa10 = ForestAge_TC010, fa20 = ForestAge_TC020, fa30 = ForestAge_TC030, fastd = LastTimeTCloss_std, faint = TCloss_intensity) %>%
    dplyr::select(-pred_burn_depth) %>%
    rename(pred_burn_depth = pred_burn_depth_LCC_11_18)

lcc_sa_fieldComputedHybrid
```



```{r}
lcc_sa_fieldComputedHybrid %>% filter(is.na(hybridAge))
lcc_sa_fieldComputedHybrid %>% filter(is.na(fieldAge))
lcc_sa_fieldComputedHybrid %>% filter(is.na(computedAge))
```
...
```{r eval=FALSE, include=FALSE}
write.csv(lcc_sa_fieldComputedHybrid, "../outputs/LCC_cleaned/csv/2025-12-02_LCC_SpatialData.csv", row.names=F)
```
