...
( lc_site = lc_id %>% 
    dplyr::select(ID:plot, unique_site_id, official_fire_scar, latitude, longitude, recent_burn, combustion_above, burn_depth, total_c_combusted, reburn_age_original_estimate) %>% 
    ...
    summarise(
      ...
      standAge = mean(reburn_age_original_estimate)
    ) %>%
    arrange(site) %>%
    ungroup() %>%
    mutate(project = "LC",
...
```
```{r}
write_csv(lc_site, "../outputs/LCC_cleaned/csv/2025-11-18_LC_Site_fullCombustionID.csv")
