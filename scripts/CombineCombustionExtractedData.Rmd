...
```
```{r}
fisl_fwi_gfwed = read_csv("../outputs/LCC_cleaned/csv/2025-06-11_Fisl_fwi_gfwed.csv")
```
...
```
```{r}
lc_fwi_gfwed = read_csv("../outputs/LCC_cleaned/csv/2025-06-11_LC_fwi_gfwed.csv")
```
...
```{r}
lcc_fwi_gfwed = read_csv("../outputs/LCC_cleaned/csv/2025-06-11_LCC_fwi_gfwed.csv")
```
## Field data
```{r}
...
# Finalize LC and FISL for combustion model

```{r}
( fisl_for_model = fisl_df1 %>% select(-lat, -lon) %>% drop_na() )
( lc_for_model = lc_df1 %>% select(-lat, -lon) %>% drop_na() )
```

```{r}
write_csv(lc_for_model, "../outputs/LCC_cleaned/csv/2025-06-23_LC_forModel.csv")
write_csv(fisl_for_model, "../outputs/LCC_cleaned/csv/2025-06-23_FISL_forModel.csv")
```
