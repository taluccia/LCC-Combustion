...
```{r}
( fisl_oldrdnbr = csv_rbind(fisl_oldrdnbr_list) ) 
( lc_oldrdnbr = csv_rbind(lc_oldrdnbr_list) ) 
```
...
```{r}
( fisl_oldrdnbr_clean = fisl_oldrdnbr %>% clean_rbind() %>% select(ID, rdnbr) %>% rename(rdnbr_old = rdnbr) )
( lc_oldrdnbr_clean =  lc_oldrdnbr %>% clean_rbind() %>% select(ID, rdnbr) %>% rename(rdnbr_old = rdnbr) )
```
...
```{r}
write_csv(fisl_oldrdnbr_clean, "../outputs/LCC_cleaned/csv/2025-07-07_FISL_oldrdnbr.csv")
write_csv(lc_oldrdnbr_clean, "../outputs/LCC_cleaned/csv/2025-07-07_LC_oldrdnbr.csv")
```
