...
```{r}
lc_points = st_read("../outputs/dob_na/lc_dob_na_2025-06-06.gpkg")
lcc_points = st_read("../outputs/dob_na/lcc_dob_na_2025-06-06.gpkg")
```
```{r}
lcc_points
```
...
```{r}
sort(unique(lc_points$fireYr))
sort(unique(lcc_points$fireYr))
```

## Fire Atlas data

```{r}
gpkg_files <- list.files("../data/fireAtlas/2022/Snapshot", pattern='.gpkg$', full.names=TRUE)
```
...
```{r}
( points_subset = lcc_points %>% filter(fireYr==2022) %>% st_transform(., crs=3571) )
```
...
st_write(joined_points_min_dob, "../outputs/fireatlas_dob/lcc_fireAtlas_dob_2022.shp", driver="ESRI Shapefile")
```
